...
function HeaderSliderButton(props) {
  ...
  function changeButtonVisibilityHandler() {
    setOpen((prev) => !prev);
  }
  return (
    <button onMouseEnter={() => changeButtonVisibilityHandler} className={`flex flex-nowrap items-center z-10 text-xl outline-none border-none w-[10.5rem] h-[3.125rem] absolute ${right} hover:opacity-90 transition-all duration-500 ${props.top}`}>
